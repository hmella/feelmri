...
# The readout handoff
# ---------------------------------------------------------------------------
def _readout_fixture(tmp_path, tag, n_side=3):
  """A small tet phantom plus a Cartesian trajectory, for the handoff tests."""
  import meshio
  from feelmri.Phantom import FEMPhantom

  g = np.linspace(-0.05, 0.05, n_side)
  P = np.array([[x, y, z] for x in g for y in g for z in g], dtype=np.float64)
  n = P.shape[0]
  rng = np.random.default_rng(4)
  cells = []
  for _ in range(6 * (n_side - 1) ** 3):
    cells.append(rng.choice(n, size=4, replace=False))
  ...
  ph.set_assembler(voxel_size=1e3, lorder=2, nodal_approximation=True,
                   lumped=True)
  ph.set_static_fields(T2=np.full(n, 120.0, dtype=np.float32),
                       phi_dB0=np.zeros(n, dtype=np.float32))
  return ph, n


@pytest.mark.parametrize('with_field', [False, True])
def test_readout_reproduces_the_hand_written_handoff(tmp_path, with_field):
  """`phantom.readout` must equal what the examples spell out by hand.

  The comparison arm is written the way the shipped examples write it --
  `traj.times - traj.t_start`, `b0_shifted_points`, `uniform_phi_rate` folded
  into `phi_dB0` -- so this pins the helper against the established idiom
  rather than against itself.
  """
  from pint import Quantity as Q_
  from feelmri import B0Field, CartesianStack, Scanner

  scanner = Scanner()
  ph, n = _readout_fixture(tmp_path, f'readout_{int(with_field)}.vtu')
  traj = CartesianStack(FOV=Q_(np.array([0.2, 0.2, 0.01]), 'm'),
                        res=np.array([8, 4, 1]), oversampling=1,
                        scanner=scanner, t_start=Q_(1.5, 'ms'))
  Mxy = (np.linspace(0.2, 1.0, n) + 0.3j).astype(np.complex64)

  field = None
  if with_field:
    field = B0Field(Q_(2.0e-4, 'mT'),
                    Q_(np.array([0.008, -0.003, 0.0]), 'mT/m'))

  class _Solver:                     # what `readout` reads off a BlochSolver
    b0_field = field
  ...
  got = np.asarray(ph.readout(Mxy, traj, pod=None, solver=solver)).reshape(-1)

  # The hand-written arm.
  t = traj.times.m_as('ms') - traj.t_start.m_as('ms')
  points = traj.points
  if field is not None:
    points = traj.b0_shifted_points(field, scanner)
    ph.set_static_fields(
        T2=np.full(n, 120.0, dtype=np.float32),
        phi_dB0=np.full(n, field.uniform_phi_rate(scanner, location=traj.LOC),
                        dtype=np.float32))
  ph.update_magnetization(Mxy)
  want = np.asarray(ph.mri_signal(list(points), t, None)).reshape(-1)

  scale = max(np.abs(want).max(), 1e-30)
  assert np.abs(got - want).max() / scale < 1e-5


def test_readout_refuses_a_per_node_field_by_name(tmp_path):
  """A field needing the per-node expansion rides the PHANTOM, not the
  trajectory. Returning the nominal trajectory instead would be a wrong image
  with no symptom, so it is refused and the message says what to call."""
  from pint import Quantity as Q_
  from feelmri import B0Field, CartesianStack, Scanner

  scanner = Scanner()
  ph, n = _readout_fixture(tmp_path, 'readout_nodal.vtu')
  traj = CartesianStack(FOV=Q_(np.array([0.2, 0.2, 0.01]), 'm'),
                        res=np.array([8, 4, 1]), oversampling=1,
                        scanner=scanner, t_start=Q_(1.5, 'ms'))
  rough = lambda p: 1e-3 * np.sin(p[:, 0] / 0.03)
  field = B0Field.on_phantom(rough, ph, collective=False)
  assert field.kind == 'nodal'
  ...
  with pytest.raises(ValueError, match='needs a per-node expansion'):
    ph.readout(np.ones(n, dtype=np.complex64), traj, solver=solver)


def test_readout_measures_time_from_the_snapshot(tmp_path):
  """The defect this helper exists to close: passing `traj.times` instead of
  `traj.times - traj.t_start` applies a spurious `exp(-t_start/T2)` and, with
  any off-resonance, a spatially varying phase ramp. The two must differ, or
  the helper is not doing the subtraction."""
  from pint import Quantity as Q_
  from feelmri import CartesianStack, Scanner

  scanner = Scanner()
  ph, n = _readout_fixture(tmp_path, 'readout_origin.vtu')
  traj = CartesianStack(FOV=Q_(np.array([0.2, 0.2, 0.01]), 'm'),
                        res=np.array([8, 4, 1]), oversampling=1,
                        scanner=scanner, t_start=Q_(40.0, 'ms'))
  Mxy = np.ones(n, dtype=np.complex64)
  ...
  got = np.asarray(ph.readout(Mxy, traj, solver=solver)).reshape(-1)

  ph.update_magnetization(Mxy)
  absolute = np.asarray(
      ph.mri_signal(list(traj.points), traj.times.m_as('ms'), None)).reshape(-1)
