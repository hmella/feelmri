...
if __name__ == '__main__':
  ...
  # # Bloch solver
  # solver = BlochSolver(seq, phantom, scanner=scanner, M0=1, T1=Q_(parameters.T1, 's'), T2=Q_(parameters.T2star, 's'), delta_B=delta_B0.reshape((-1, 1)))
  ...
  if MPI_rank == 0:
    plotter = MRIPlotter(images=[m, phi_x, phi_y, phi_z, phi_ref], title=['Magnitude', 'phi_x', 'phi_y', 'phi_z', 'phi_ref'])
    plotter.show()