...
if __name__ == '__main__':
  ...
  # Show reconstruction
  m = np.abs(I[...,0,:])
  phi_vx = np.angle(I[...,0,:] * np.conj(I[...,3,:]))
  phi_vy = np.angle(I[...,1,:] * np.conj(I[...,3,:]))
  phi_vz = np.angle(I[...,2,:] * np.conj(I[...,3,:]))
  phi_ref = np.angle(I[...,3,:])
  if MPI_rank == 0:
    plotter = MRIPlotter(images=[m, phi_vx, phi_vy, phi_vz, phi_ref], title=['Magnitude', '$\\phi_{vx}$ ', '$\\phi_{vy}$', '$\\phi_{vz}$', '$\\phi_{ref}$'], swap_axes=[0, 2], FOV=planning.FOV.m_as('m'))
    plotter.show()