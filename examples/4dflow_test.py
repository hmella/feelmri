...
from feelmri.MPIUtilities import MPI_print, MPI_rank, MPI_comm, gather_data
...
if __name__ == '__main__':
  ...
  def spatial(x):
      return x[:,0] + x[:,1] + x[:,2]
  lab_nodes = phantom.local_nodes @ planning.MPS.T + planning.LOC.m_as('m')
  peak = MPI_comm.allreduce(float(np.abs(spatial(lab_nodes)).max()), op=MPI.MAX)
  ...
  phi_ref = f * np.angle(Im[...,3,:])
  phi_vx = f * np.angle(Im[...,0,:] * np.conj(Im[...,3,:]))
  phi_vy = f * np.angle(Im[...,1,:] * np.conj(Im[...,3,:]))
  phi_vz = f * np.angle(Im[...,2,:] * np.conj(Im[...,3,:]))
  if MPI_rank == 0:
    plotter = MRIPlotter(images=[mag, phi_vx, phi_vy, phi_vz, phi_ref], title=['Magnitude', '$\\phi_{vx}$ ', '$\\phi_{vy}$', '$\\phi_{vz}$', '$\\phi_{ref}$'], FOV=planning.FOV.m_as('m'), swap_axes=[0, 2])
    plotter.show()

  # Prepare data for 4D flow toolbox
  FOV = planning.FOV.m_as('m')
  resolution = np.array(pars.Imaging.ACQ_RES)
  mx = np.abs(Im[...,0,:])
  my = np.abs(Im[...,1,:])
  mz = np.abs(Im[...,2,:])
  phi_x = np.angle(Im[...,0,:] * np.conj(Im[...,3,:]))
  phi_y = np.angle(Im[...,1,:] * np.conj(Im[...,3,:]))
  phi_z = np.angle(Im[...,2,:] * np.conj(Im[...,3,:]))
  phi_ref = np.angle(Im[...,3,:])

  VENC_cms = pars.VelocityEncoding.VENC.m_as('cm/s')
  ndata = {'MR_FFE_FH': mz,
          'MR_FFE_AP': mx,
          'MR_FFE_RL': my,
          'MR_PCA_FH': VENC_cms/np.pi*phi_z,
          'MR_PCA_AP': VENC_cms/np.pi*phi_x,
          'MR_PCA_RL': VENC_cms/np.pi*phi_y,
          'voxel_MR': 1000.0*(FOV/resolution).reshape((1, 3)),
          'VENC': VENC_cms,
          'heart_rate': 60.0,
          'type': 'DCM'}
