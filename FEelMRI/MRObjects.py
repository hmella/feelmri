...
# Gradient
class Gradient:
  def __init__(self, slope=None, lenc=Q_(1.0,'ms'), G=None, scanner=Scanner(), t_ref=Q_(0.0,'ms')):
  ...
  def make_bipolar(self, VENC):
    ...
    # Upadte duration
    if self.lenc < 0:
      self.dur = 2*(self.slope + self.slope).to('ms')
    else:
      self.dur = 2*(self.slope + self.lenc + self.slope).to('ms')

    # Update timings and amplitudes in array
    _, timings, amplitudes, _ = self.group_timings()

    # Concatenate gradient lobes
    ...
    self.interpolator = interp1d(self.timings.m, self.amplitudes.m, kind='linear', fill_value=0.0, bounds_error=False)
