...
class Gradient:
  ...
  def make_bipolar(self, VENC):
    ...
    slope_ = self._Gr_max/self._Gr_sr # [s]
    slope_req_ = np.cbrt(np.pi/(2*self._gamma*self._Gr_sr*VENC))
    ...
    if slope_req_ <= slope_:
      ...
      a = self._gamma*VENC*self._Gr_max
      b = 3*self._gamma*VENC*self._Gr_max*slope_
      c = 2*self._gamma*VENC*self._Gr_max*slope_**2 - np.pi
      lenc_req_= (-b + np.sqrt(b**2 - 4*a*c))/(2*a) # [s]

      # Gradients parameters
      self._slope = slope_
      self._G     = -self._Gr_max
      self._lenc  = lenc_req_
...
class Cartesian(Trajectory):
    ...
    def kspace_points(self):
      ...
      ph_grad = Gradient(t_ref=0.0, Gr_max=self.Gr_max, Gr_sr=self.Gr_sr)
      ...
      enc_time = self.G_enc.timings[-1] - np.max([ph_grad.dur, ro_grad0.dur]) # ms
      ...
      for shot in self.shots:
        for idx, ph in enumerate(shot):
          ...
          if idx == 0:
            ...
            t[::ro,ph] = t[:,shot[idx-1]].max() + ro_grad._slope + ro_grad._slope + dt[::ro]
